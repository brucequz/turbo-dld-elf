...
GaloisField::GaloisField(const int pwr, const unsigned int *primitive_poly)
  ...
  for (unsigned int i = 0; i < power; i++) {
    // std::cout << "primitive_poly[i]" << primitive_poly[i] << std::endl;
  ...
  */
  ...
  for (unsigned int i = 0; i < field_size + 1; i++) {
    for (unsigned int j = 0; j < field_size + 1; j++) {
      ...
      // std::cout << i << " "<< index_of[i] << " " << j <<  " "<<  index_of[j]
      // << std::endl;
...
GFSymbol GaloisField::gen_mul(const GFSymbol &a, const GFSymbol &b) {
  ...
  // return alpha_to[fast_modulus(log(a) + log(b))];
}
...
GFSymbol GaloisField::gen_div(const GFSymbol &a, const GFSymbol &b) {
  if ((a == 0) || (b == 0))
    ...
    // return alpha_to[fast_modulus(log(a) - log(b) + field_size)];
  }
}
...
GFSymbol GaloisField::gen_exp(const GFSymbol &a, const unsigned int &n) {
  if (a != 0) {
    ...
    // return alpha_to[fast_modulus(log(a) * n)];
  } else
    return 0;
}
