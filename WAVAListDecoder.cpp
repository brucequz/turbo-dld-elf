...
ListDecoder::constructWAVATrellis(std::vector<double> receivedMessage,
                                  std::vector<std::vector<cell>> oneTrellis) {
  ...
  for (int i = 0; i < numStates / 2; i++) {
    trellisInfo[i][0].pathMetric =
        oneTrellis[i][oneTrellis[i].size() - 1].pathMetric;
...
ListDecoder::modifiedWAVADecoding(std::vector<double> receivedMessage) {
  ...
  std::vector<std::vector<cell>> trellisInfo;
  std::vector<std::vector<cell>> oneTrellis =
      constructOneTrellis(receivedMessage);
...
ListDecoder::messageInformation
ListDecoder::wavaDecoding(std::vector<double> receivedMessage) {
  pathLength = receivedMessage.size() + 1;
